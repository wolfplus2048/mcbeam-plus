// Package grpc provides a grpc mcb_server
package grpc
...
var (
	// DefaultMaxMsgSize define maximum message size that mcb_server can send
...
func (g *grpcServer) handler(srv interface{}, stream grpc.ServerStream) error {
	...
	// timeout for mcb_server deadline
	to := md["timeout"]
	...
	if g.opts.Router != nil {
		...
		if err != nil {
			return errors.InternalServerError("go.micro.mcb_server", err.Error())
		}
		codec := &grpcCodec{
...
func (g *grpcServer) processRequest(stream grpc.ServerStream, service *service, mtype *methodType, ct string, ctx context.Context) error {
	for {
		...
		if mtype.ReplyType == nil {
			replyv = reflect.New(reflect.TypeOf(Message{}))
		} else {
		...
		fn := func(ctx context.Context, req server.Request, rsp interface{}) (err error) {
			defer func() {
				if r := recover(); r != nil {
					...
					err = errors.InternalServerError("go.micro.mcb_server", "panic recovered: %v", r)
				}
			}()
			if mtype.ReplyType != nil {
...
func (g *grpcServer) Register() error {
	...
	node.Metadata["mcb_server"] = g.String()
...
func (g *grpcServer) Start() error {
	...
	// mark the mcb_server as started
	g.Lock()
	g.started = true
	g.Unlock()
