...
// mcb_server represents an RPC Server.
type rServer struct {
...
func prepareEndpoint(method reflect.Method) *methodType {
	...
	if stream {
		...
		if replyType != nil && !isExportedOrBuiltinType(replyType) {
			if logger.V(logger.ErrorLevel, logger.DefaultLogger) {
	...
	for m := 0; m < s.typ.NumMethod(); m++ {
		...
		if mt := prepareEndpoint(method); mt != nil {
			...
			if s.name != "Debug" {
				if logger.V(logger.ErrorLevel, logger.DefaultLogger) {
					if mt.ReplyType != nil {
						logger.Debugf("registered component %s.%s(arg:%s, rsp:%s)", s.name, method.Name, mt.ArgType.Elem().Name(), mt.ReplyType.Elem().Name())

					} else {
						logger.Debugf("registered component %s.%s(arg:%s)", s.name, method.Name, mt.ArgType.Elem().Name())
